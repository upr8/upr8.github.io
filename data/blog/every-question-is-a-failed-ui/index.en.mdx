...
There is a caveat. The users who open a chat are a self-selected group. They may skew toward certain demographics, comfort levels, or frustration thresholds. Optimising the UI purely based on what chat users ask risks a selection bias — you end up designing for the segment that asks, not for everyone. Chat data is a powerful signal, but it is not the only signal, and it should be triangulated with broader behavioural data before driving major design decisions.
